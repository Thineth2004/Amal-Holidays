...
const StaffPage: React.FC<StaffPageProps> = ({ role, title, endpoint }) => {
  ...
  const executeDeleteStaff = async () => {
    ...
    try {
      await api.delete(`/api/${endpoint}/${confirmDeleteId}`);
      setStaffList(staffList.filter((s) => s.user_id !== confirmDeleteId));
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-transparent font-['Plus_Jakarta_Sans'] antialiased">
      <ConfirmDeleteModal
        ...
        title={`Delete ${title}`}
        message={`Are you sure you want to delete ${title.toLowerCase()} #${confirmDeleteId}? This action cannot be undone.`}
      />
                  ...
                  <tr key={s.user_id} className="group hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-8 py-6 text-right align-top">
                      <div className="flex items-center justify-end gap-3 mt-4">
                        ...
                        <button onClick={() => setConfirmDeleteId(s.user_id)} className="flex items-center gap-2 px-4 py-2 text-red-500 bg-white border border-slate-200 hover:bg-red-50 rounded-xl transition-all font-bold text-xs">
